...
public class CardsController : ControllerBase
{
    ...
    public async Task<IActionResult> ImportCards([FromForm] CardImportRequestDto request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            var uploader = string.IsNullOrWhiteSpace(request.ImportedBy) ? "POS_USER" : request.ImportedBy;
            var result = await _cardImportService.ImportCardsCsvAsync(request.File, uploader, cancellationToken);
            return Ok(ApiResponse<CardImportResultDto>.Ok(result, "CSV file processed successfully."));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Failed to import card CSV file: {FileName}", request.File.FileName);
